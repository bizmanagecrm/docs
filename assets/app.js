(() => {
  ...
  function renderApiCatalog(host) {
    ...
    const families = [...new Set(all.map((item) => item.family).filter(Boolean))];
    const permissions = [...new Set(all.map((item) => item.permission))];
    const impacts = [...new Set(all.map((item) => item.impact))];
    const views = [...new Map(all.filter((item) => item.view).map((item) => [item.view, item.viewDisplay || item.view])).entries()];
    const operations = [...new Set(all.map((item) => item.operation).filter(Boolean))].sort();
    host.innerHTML = `<div class="filter-bar api-filter-bar"><input type="search" placeholder="Search endpoint, view, operation, or purpose" aria-label="Filter API endpoints"><select aria-label="Filter by route family"><option value="">All route families</option>${families.map((family) => `<option value="${escapeHtml(family)}">${escapeHtml(family)}</option>`).join('')}</select><select aria-label="Filter by view"><option value="">All configured views</option>${views.map(([value, label]) => `<option value="${escapeHtml(value)}">${escapeHtml(label)} · ${escapeHtml(value)}</option>`).join('')}</select><select aria-label="Filter by operation"><option value="">All CRUD operations</option>${operations.map((operation) => `<option value="${escapeHtml(operation)}">${escapeHtml(operation)}</option>`).join('')}</select><select aria-label="Filter by permission"><option value="">All permission levels</option>${permissions.map((permission) => `<option value="${escapeHtml(permission)}">${escapeHtml(permission)}</option>`).join('')}</select><select aria-label="Filter by impact"><option value="">All impacts</option>${impacts.map((impact) => `<option value="${escapeHtml(impact)}">${escapeHtml(impact)}</option>`).join('')}</select></div><p class="catalog-count"></p><div class="endpoint-list"></div>`;
    const input = host.querySelector('input'); const [familySelect, viewSelect, operationSelect, permissionSelect, impactSelect] = host.querySelectorAll('select'); const list = host.querySelector('.endpoint-list'); const count = host.querySelector('.catalog-count');
    const update = () => {
      const term = input.value.trim().toLowerCase();
      const filtered = all.filter((item) => (!familySelect.value || item.family === familySelect.value) && (!viewSelect.value || item.view === viewSelect.value) && (!operationSelect.value || item.operation === operationSelect.value) && (!permissionSelect.value || item.permission === permissionSelect.value) && (!impactSelect.value || item.impact === impactSelect.value) && (!term || `${item.method} ${item.endpoint} ${item.family} ${item.view || ''} ${item.viewDisplay || ''} ${item.operation || ''} ${item.permission} ${item.impact} ${item.purpose} ${item.request || ''} ${item.response || ''} ${item.notes || ''}`.toLowerCase().includes(term)));
      list.innerHTML = filtered.slice(0, 200).map((item) => `<article class="endpoint-card"><div class="endpoint-card-head"><span class="tag ${item.method.toLowerCase()}">${escapeHtml(item.method)}</span><code>${escapeHtml(item.endpoint)}</code><span class="tag permission-${item.permission.toLowerCase().replace(/\s+/g, '-')}">${escapeHtml(item.permission)}</span><span class="endpoint-impact">${escapeHtml(item.impact)}</span></div><p>${escapeHtml(item.purpose)}</p><div class="endpoint-details"><div><strong>Request body</strong><code class="code-value">${escapeHtml(item.request || 'Not documented for this route.')}</code></div><div><strong>Response</strong><span>${escapeHtml(item.response || 'JSON response.')}</span></div></div>${item.notes ? `<p class="endpoint-note"><strong>Important:</strong> ${escapeHtml(item.notes)}</p>` : ''}</article>`).join('');
      count.textContent = `${filtered.length.toLocaleString()} of ${all.length.toLocaleString()} documented endpoints${filtered.length > 200 ? ' · showing first 200; use a view, operation, or search filter' : ''}`;
    };
    input.addEventListener('input', update); [familySelect, viewSelect, operationSelect, permissionSelect, impactSelect].forEach((select) => select.addEventListener('change', update)); update();
  }
  ...
  function searchableItems() {
    ...
    const api = catalogData.api.map((item) => ({ href: '#/api', title: `${item.method} ${item.endpoint}`, meta: `${item.permission} · ${item.impact} · ${item.purpose}`, search: `${item.method} ${item.endpoint} ${item.family || ''} ${item.view || ''} ${item.viewDisplay || ''} ${item.operation || ''} ${item.permission} ${item.impact} ${item.purpose} ${item.request || ''} ${item.notes || ''}`.toLowerCase() }));
